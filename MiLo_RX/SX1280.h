...
bool  IRAM_ATTR SX1280_Begin(void);
...
void  IRAM_ATTR SX1280_Reset(void);
...
void IRAM_ATTR SX1280_SetPacketParamsLoRa(uint8_t PreambleLength, uint8_t HeaderType,uint8_t PayloadLength, uint8_t crc,uint8_t InvertIQ);
//void IRAM_ATTR SX1280_SetDioIrqParams( uint16_t irqMask, uint16_t dio1Mask, uint16_t dio2Mask, uint16_t dio3Mask );
void IRAM_ATTR SX1280_SetDioIrqParams(uint16_t irqMask,
...
uint8_t  IRAM_ATTR SX1280_GetStatus(void);
int8_t IRAM_ATTR SX1280_GetRssiInst( void );
void IRAM_ATTR SX1280_GetLastPacketStats(void);
uint8_t IRAM_ATTR SX1280_GetPacketType(void);
bool IRAM_ATTR WaitOnBusy(void);
...
void  IRAM_ATTR getRFlinkInfo(void);
void  IRAM_ATTR POWER_init(void);
void  IRAM_ATTR SX1280_setPower(uint8_t Power);
void  IRAM_ATTR SX1280_SetOutputPower( int8_t power );
int32_t IRAM_ATTR SX1280_GetLoRaBandwidth(void);
int32_t IRAM_ATTR complement2( const uint32_t num, const uint8_t bitCnt );
double IRAM_ATTR SX1280_GetFrequencyError(void);
